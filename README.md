...
Place trained model weights in `~/Downloads/` — the bridge script expects exactly these 9 items:

```
~/Downloads/
├── detector_cnn_bilstm_attn_ft/        ← CNN-BiLSTM-Attn detector weights (folder)
├── detector_cnn_attention_ft/          ← CNN-Attn detector weights (folder)
├── detector_cnn_only_ft/               ← CNN-only detector weights (folder)
├── rl_policy_best_ACCEPTED_cnn_bilstm_attn_ft/   ← policy weights (folder)
├── rl_policy_best_ACCEPTED_cnn_attn_ft/
├── rl_policy_best_ACCEPTED_cnn_only_ft/
├── scaler_cnn_bilstm_attn_ft.pkl       ← feature scaler (file)
├── scaler_cnn_attention_ft.pkl
└── scaler_cnn_only_ft.pkl
```

> **Note:** These weights are not included in this repository. They were trained separately
> and are stored on the author's machine at `~/Downloads/`.
> Without the weights the bridge falls back to a rule-based fallback (no AI inference).
